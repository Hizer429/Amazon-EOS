  ...
  function parseCSV(filePath) {
    fs.readFile(filePath, 'utf8', (err, data) => {
      ...
      lines.forEach((line, i) => {
        ...
        if (["UPSS", "FDEG"].includes(carrier) && location.startsWith("DD"))
          parcelsDock++;

        if (["UPSS", "FDEG"].includes(carrier) && location.startsWith("PS"))
          parcelsYard++;

        if (appointment === "TRANSSHIP" && location.startsWith("PS"))
          transshipYard++;

        if (carrier === "AZNG" && dwell >= 72)
          azngOver72++;
